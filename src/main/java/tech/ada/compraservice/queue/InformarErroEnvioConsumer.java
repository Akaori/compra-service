...
import tech.ada.compraservice.service.TratarErroEnvioService;
...
@Slf4j
public class InformarErroEnvioConsumer {

    private final ObjectMapper objectMapper;
    private final TratarErroEnvioService tratarErroEnvioService;

    @RabbitListener(queues = {"${negocio.envio.fila}"})
    public void consumer(Message message , Channel channel)  {
        try {
            ...
            log.info("Mensagem de erro do Envio consumida {}", informarErro);
            tratarErroEnvioService.execute(informarErro);
